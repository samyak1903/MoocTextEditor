...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		{
			if(i==ch.length-1&& Character.toLowerCase(ch[i])=='e'&& numsyllables>0 && newsyllable)
			{
				numsyllables--;
			}
			if(vowels.indexOf(Character.toLowerCase(ch[i]))>=0 && newsyllable)
			{
				numsyllables++;
				newsyllable=false;
	...
	public double getFleschScore()
	{
	    ...
	    return 206.835-1.015*((double)getNumWords()/getNumSentences())-84.6*((double)getNumSyllables()/(double)getNumWords());
	}
